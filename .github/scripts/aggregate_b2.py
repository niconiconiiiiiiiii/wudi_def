...
def fetch_chara_names():
    ...
    for url in urls:
        try:
            ...
            print(f"角色别名源 {url} 加载失败：{exc}")
    print("所有角色别名源加载失败，将使用角色 ID")
    return {}
...
def translate_team(team, nicknames):
    ...
    for raw_unit_id in team:
        ...
        if aliases:
            names.append(str(aliases[0]) if isinstance(aliases, list) else str(aliases))
        else:
...
def write_daily_reports(result, nicknames):
    ...
    with strength_path.open("w", encoding="utf-8-sig", newline="") as file:
        ...
        for rate, sample in filtered_strengths:
            tested_range = f"{sample.get('tested_min', sample['tested_count'])}~{sample.get('tested_max', sample['tested_count'])}"
            writer.writerow(
                [
                    f"{rate * 100:.1f}%",
                    sample.get("bot_count", 1),
                    tested_range,
