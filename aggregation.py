    ...
    for (_, kind), snapshot in latest.items():
        ...
        for sample in snapshot["payload"]["strong_defenses"]:
            ...
            if key not in merged:
                ...
                merged[key]["bot_count"] = 1
                merged[key]["tested_min"] = int(sample.get("tested_count", 0))
                merged[key]["tested_max"] = int(sample.get("tested_count", 0))
                continue
            current = merged[key]
            ...
            current["bot_count"] = current.get("bot_count", 1) + 1
            tc = int(sample.get("tested_count", 0))
            current["tested_min"] = min(current.get("tested_min", tc), tc)
            current["tested_max"] = max(current.get("tested_max", tc), tc)
